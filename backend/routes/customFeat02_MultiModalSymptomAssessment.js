...
// Combine audio (patient speech), video (facial grimacing analysis), vital trends; holistic triage score more accurate than ESI alone
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / cf-multi-modal-symptom-assessment',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_multi_modal_symptom_assessment`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-multi-modal-symptom-assessment feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Multi-modal symptom assessment'.
Context: Combine audio (patient speech), video (facial grimacing analysis), vital trends; holistic triage score more accurate than ESI alone
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-multi-modal-symptom-assessment/run error:', err);
