...
// Gap Feature: Discharge without \'/readmission
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / gap-discharge-without-readmission',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Discharge without \'/readmission'.\nProject: AIPatientTriageforER.\nContext: Discharge without /readmission-risk (flag high-risk discharges for extra follow-up)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['discharge-without-readmission', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'discharge-without-readmission', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
