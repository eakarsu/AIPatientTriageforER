...
// Gap Feature: No ambulance/EMS integration (arrival notifications, field triage data)
// No ambulance/EMS integration (arrival notifications, field triage data)
// Project: AIPatientTriageforER
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / gap-no-ambulance-ems-integration-arrival-notifications',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No ambulance/EMS integration (arrival notifications, field triage data)'.\nProject: AIPatientTriageforER.\nContext: No ambulance/EMS integration (arrival notifications, field triage data)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-ambulance-ems-integration-arrival-notifications', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-ambulance-ems-integration-arrival-notifications', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-ambulance-ems-integration-arrival-notifications']
    );
