...
// Gap Feature: No production
// No production-grade EHR integration (Epic, Cerner) despite integrations.js stub
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / gap-no-production',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No production'.\nProject: AIPatientTriageforER.\nContext: No production-grade EHR integration (Epic, Cerner) despite integrations.js stub\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-production', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-production', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
