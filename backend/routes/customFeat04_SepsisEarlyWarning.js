...
// Real-time vital streaming with AI ensemble (qSOFA, NEWS, SIRS, lactate trends); push alerts 30-60 min before sepsis becomes critical
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / cf-sepsis-early-warning',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_sepsis_early_warning`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-sepsis-early-warning feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Sepsis early warning'.
Context: Real-time vital streaming with AI ensemble (qSOFA, NEWS, SIRS, lactate trends); push alerts 30-60 min before sepsis becomes critical
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-sepsis-early-warning/run error:', err);
