...
// Continuous background process identifies patients safe for discharge; alerts physicians proactively with suggested discharge orders
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / cf-discharge-risk-stratification',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_discharge_risk_stratification`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-discharge-risk-stratification feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Discharge risk stratification'.
Context: Continuous background process identifies patients safe for discharge; alerts physicians proactively with suggested discharge orders
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-discharge-risk-stratification/run error:', err);
