...
// Gap Feature: No notifications layer dedicated to clinical alerts
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatientTriageforER / gap-no-notifications-layer-dedicated-to-clinical-alert',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No notifications layer dedicated to clinical alerts'.\nProject: AIPatientTriageforER.\nContext: No notifications layer dedicated to clinical alerts\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-notifications-layer-dedicated-to-clinical-alert', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-notifications-layer-dedicated-to-clinical-alert', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-notifications-layer-dedicated-to-clinical-alert']
    );
