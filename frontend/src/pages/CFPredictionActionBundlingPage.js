...
export default function CFPredictionActionBundlingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-prediction-action-bundling/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Prediction + action bundling</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Prediction + action bundling: When AI predicts patient needs (e.g., \"likely needs CT + orthopedics\"), auto-order tests and notify departments before patient reaches them</p>
