...
// Gap (Non-AI): No webhooks for critical alerts to pagers/phones
// No webhooks for critical alerts to pagers/phones
import React, { useState } from 'react';

export default function GapNoWebhooksForCriticalAlertsToPagersPhonesPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-critical-alerts-to-pagers-phones/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for critical alerts to pagers/phones</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for critical alerts to pagers/phones</p>
