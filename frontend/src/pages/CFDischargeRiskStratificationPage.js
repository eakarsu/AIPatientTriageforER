...
// Custom Feature: Discharge risk stratification
// Discharge risk stratification: Continuous background process identifies patients safe for discharge; alerts physicians proactively with suggested discharge orders
import React, { useState } from 'react';

export default function CFDischargeRiskStratificationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-discharge-risk-stratification/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Discharge risk stratification</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Discharge risk stratification: Continuous background process identifies patients safe for discharge; alerts physicians proactively with suggested discharge orders</p>
