...
// Custom Feature: Sepsis early warning
// Sepsis early warning: Real-time vital streaming with AI ensemble (qSOFA, NEWS, SIRS, lactate trends); push alerts 30-60 min before sepsis becomes critical
import React, { useState } from 'react';

export default function CFSepsisEarlyWarningPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-sepsis-early-warning/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Sepsis early warning</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Sepsis early warning: Real-time vital streaming with AI ensemble (qSOFA, NEWS, SIRS, lactate trends); push alerts 30-60 min before sepsis becomes critical</p>
