...
// Gap (AI): Resources without '/staffing
// Resources without /staffing-optimize (predict staffing needs 6-12 hours ahead)
import React, { useState } from 'react';

export default function GapResourcesWithoutStaffingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-resources-without-staffing/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Resources without '/staffing</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Resources without /staffing-optimize (predict staffing needs 6-12 hours ahead)</p>
