...
// Gap (Non-AI): No ambulance/EMS integration (arrival notifications, field triage data)
// No ambulance/EMS integration (arrival notifications, field triage data)
import React, { useState } from 'react';

export default function GapNoAmbulanceEmsIntegrationArrivalNotificationsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-ambulance-ems-integration-arrival-notifications/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No ambulance/EMS integration (arrival notifications, field triage data)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No ambulance/EMS integration (arrival notifications, field triage data)</p>
